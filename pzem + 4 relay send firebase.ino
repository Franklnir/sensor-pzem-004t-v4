/*
 * === ESP32-S3 Mini — PZEM-004T v3 + Firebase + BME280/BH1750/MQ135 ===
 * Versi: MURNI TELEMETRY (DENGAN FITUR RESET kWh JARAK JAUH) + RELAY 4-CH + SCHEDULER.
 *
 * Stream Firebase:
 ...
 *  - ESP8266 relay pins default: 14,12,0,2  (ACTIVE-LOW) — sesuai snippet aslinya
 */
...
// ================== Relay (ACTIVE-LOW) ==================
// ESP8266: mempertahankan pin sesuai snippet asli
#if defined(ESP8266)
  const int relayPins[4] = {14, 12, 0, 2};
#elif defined(ESP32)
// ESP32/ESP32-S3: pakai pin aman agar tidak konflik dengan PZEM/I2C/Buzzer/LED
  const int relayPins[4] = {6, 7, 15, 16};
#endif
...
// ================== Ambang MQ135 ==================
const float MQ135_HIGH_PPM = 200.0f;
const float MQ135_LOW_PPM  = 180.0f;
...
void setup(){
  ...
  // Mulai stream:
  // - commands (reset_kwh) => via streamCmd (sudah diset di init)
  // - relay folder          => streamRelays
  if (baseRelaysPath.length()) beginStreamWithRetry(baseRelaysPath);

  Serial.println("\n======================================================");
  Serial.println(" ESP32-S3 + PZEM + Lingkungan + Relay + Scheduler");
...
void loop(){
  ...
  // ======= 1) Stream /commands: reset_kwh (pakai gaya streamAvailable() sesuai kode awal) =======
  if (Firebase.ready() && streamCmd.streamAvailable()){
    // pada node /users/{uid}/commands
    if (streamCmd.dataPath()=="/reset_kwh"){
      String asStr = streamCmd.stringData();
      // dukung boolean/string "true"
      bool trigger = (asStr=="true") || streamCmd.boolData();
      if (trigger) resetEnergyCounters();
    }
  }

  // ======= 2) Stream /relay_4_channel: perubahan state manual dari app =======
  ...
  // ======= 3) Scheduler HH:MM & days untuk relay (cek setiap menit) =======
  ...
  // ======= 5) Baca sensor lingkungan + kirim RTDB tiap 3 detik =======
  ...
  if (nowMs2 - lastSendMs >= SEND_INTERVAL_MS){
    ...
    float mq135_volt = (mq135_raw/4095.0f)*ADC_REF_VOLT;
    float mq135_ppm  = map(mq135_raw, 0, 4095, 0, 1000);

    static bool buzzerState=false;
    if (mq135_ppm > MQ135_HIGH_PPM && !buzzerState){
      digitalWrite(BUZZER_PIN, HIGH); buzzerState=true; Serial.println("🚨 Buzzer ON (ppm di atas ambang)");
    } else if (mq135_ppm < MQ135_LOW_PPM && buzzerState){
      digitalWrite(BUZZER_PIN, LOW);  buzzerState=false; Serial.println("✅ PPM normal lagi, buzzer OFF");
    }
    ...
    jsonEnv.set("mq135_raw",  mq135_raw);
    jsonEnv.set("mq135_volt", mq135_volt);
    jsonEnv.set("mq135_ppm",  mq135_ppm);
    jsonEnv.set("buzzer",     buzzerState);
